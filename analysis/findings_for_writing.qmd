...
county_days_by_event_type <- concurred_days_by_type %>% 
  group_by(event_type_description) %>% 
  summarize(days = sum(days))
...
7: Who gets blamed 
```{r}
blame_data <- read_csv("here"("data", "processed", "blame_demos.csv")) %>% 
  select(starts_with("origin")) 

# Come back to this when we have states on demos and finalize 

test <- as.list(lapply(blame_data, unique))
