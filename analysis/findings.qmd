...
fed_reg_current <- read_csv(here("data", "processed", "for_vis", "federal_register_areas_current.csv")) %>% 
  select(-1)
```
...
```{r}
# For the code and census data, see ETL folder 
fed_reg_historical <- read_csv(here("data", "processed", "for_vis", "federal_register_areas_historical.csv")) %>% 
  select(-1)
```
